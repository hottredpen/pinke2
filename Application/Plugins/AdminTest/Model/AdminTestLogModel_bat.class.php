...
class AdminTestLog_batModel extends CommonModel{
    ...
    protected $_validate = array(
        ...
        array('scene_id', 'is_scene_id_pass', '[test]不存在该场景', self::MUST_VALIDATE,'callback',self::ADMIN_TEST_BEFORE),
    ...
    protected function get_this_model_test_data(){
        $this->tmp_data['all_data']       = D('Admin','Test')->getTestData($this->tmp_data['scene_id'],$this->tmp_data['test_data_group_index'],$this->tmp_data['test_data_items_index'],$this->tmp_data['uuid']);
        $this->tmp_data['handle_action']  = D('Admin','Test')->getActionName($this->tmp_data['scene_id']);
        $this->tmp_data['success_assert'] = D('Admin','Test')->getSuccessAssert($this->tmp_data['scene_id']);
        $this->tmp_data['handle_object']  = D('Admin','Test')->getHandleObject();

        $this->tmp_data['test_title']     = D('Admin','Test')->getTestTitle($this->tmp_data['scene_id']);
    }
    ...
    protected function get_this_model_test_data_by_test_id($test_id){
        ...
        $this->tmp_data['handle_action']  = $modelTest->getActionName($scene_id);
